...
namespace SuperReminder
{
	public class ReminderDispatcher
	{
		...
		public void SnoozeReminder(ReminderInfo reminder, int minutes)
		{
			reminder.SnoozeBefore(minutes);
		...
		public void Wait()
		{
			var threads = _contexts.Select(x => x.Thread).ToArray();
			// Traverse in reverse order to prevent exception related to modified collection
